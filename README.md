...
|`all-answered` (recommended)|Used to define a value to be stored as the fields answer if all the items are incorrect. This is important because in both EGRA and EGMA subtasks, selections indicate correct answers. The `all-answered` value must also be included in the choice list. If you do not supply an `all-answered` value, the failsafe behavior is to store the first item in the choice list, but this can be misleading.|
