...
[![Download now](extras/download-button.png)](https://github.com/surveycto/timed-grid-test/raw/master/timed-grid-test.fieldplugin.zip)
...
|`rtl` (optional)| Useful for conducting the tests using Right-To-Left (RTL) languages (Arabic, Urdu, Hebrew etc). The default behaviour is to present the language direction detected by SurveyCTO. Specify `rtl = 1` to force RTL.|
