...
|`end-after` (optional)|Used to specify a limit on the number of consecutive incorrect items that can be marked from the start before being prompted to end the test early. The default is 10 items for the EGRA letter reading test and 5 for the EGRA nonword or familiar reading test, but you can specify a custom value, including 0 to disable.<br>By default, a pop-up is presented to the user and they can select whether to end the test or to continue. If they choose to end the test, they are prompted to select a last item and then they can move forward. A more strict mode is available when used in combination with the `strict` parameter set to `1`. In this strict mode, the user is notified with a pop-up and on acknowledging they are immediately taken to the next field.|
